  ...
  for (const selection of selections) {
    ...
    for (const instance of selection.instances) {
      ...
      if (assignment?.pinNumber !== undefined) {
        ...
        const pinNum = parsePinNumber(assignment.pin);
...
function parsePinNumber(pinStr: string): number | undefined {
  const trimmed = pinStr.trim().toUpperCase();
  if (/^A[0-7]$/i.test(trimmed)) return 14 + parseInt(trimmed.slice(1), 10);
  if (/^D?\d{1,2}$/i.test(trimmed)) return parseInt(trimmed.replace(/^D/i, ''), 10);
  return undefined;
}
...
/** Parse telemetry field from serial trace.
 * Supports both JSON format ("field": value) and simple format (field:value)
 */
function telemetryFieldSeries(trace: FirmwareTrace, field: string): number[] {
  const series: number[] = [];
  // Match both "field": value and field:value patterns
  const pattern = new RegExp(`(?:"${field}"|${field})\\s*:\\s*(-?\\d+(?:\\.\\d+)?)`, 'gi');
...
function checkAssertion(
  ...
  const passed = compare(actual, assertion.operator, assertion.expected, assertion.tolerance ?? 0);
...
export function validateWithRealSim(input: SimValidationInput): SimValidationResult {
