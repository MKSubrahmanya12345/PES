...
| `WIREUP_TERMINAL_ENABLED` | Master switch for the terminal dock, which spawns real commands on the machine serving Wireup |
| `WIREUP_TERMINAL_ROOTS` | `path.delimiter`-separated folders a session may run in (default: this repo and the user's home). Checked for the picker *and* the spawn, after symlink resolution |
| `WIREUP_TERMINAL_FREEFORM` | Off (default) limits commands to npm/npx/pnpm/yarn/bun/node/corepack with no shell metacharacters; on allows anything |
| `WIREUP_TERMINAL_MAX_SESSIONS`, `WIREUP_TERMINAL_MAX_LINES` | Concurrent sessions before a new one is refused, and output lines kept per session |
...
| Website | `GET /api/projects/:id/simulation` → `software`, zip at `/simulation/software.zip` | A complete Vite + React + TypeScript project generated from the same device contract as the firmware — and a **skeleton**, not a fixed dashboard: `src/surface.ts` names the blocks, their order, their bindings and the layout, `src/skeleton/registry.tsx` maps block kinds to components, and `src/skeleton/Shell.tsx` renders whatever the spec says. Statically validated (imports resolve, sources parse, every reading and control is bound); never installed or built by the pipeline |
...
### The website half: a skeleton, and a terminal that runs it

Two rules shape the generated website.

**It is a structure, not a layout.** Nothing in `src/App.tsx` decides what the page
contains — it owns the board link and hands it to `<Shell/>`. The shape lives in
`src/surface.ts`, which the user is expected to edit:

| Spec field | What it controls |
| --- | --- |
| `layout` | `grid` \| `stack` \| `tabs` \| `bare` — how the shell arranges blocks |
| `blocks[]` | one entry per section: `id`, `kind`, `title`, `fields` (telemetry it reads), `characters` (commands it sends), `span`, `enabled` |
| `kind` | a key into `src/skeleton/registry.tsx`; the shipped kinds are `metrics`, `commands`, `log`, `notes`, `status`, `slot` |

A kind the registry does not know renders as an empty `slot` block that says so — never a
crash and never a silent gap. A `fields` entry the contract does not declare renders as an
*unbound* card rather than a made-up number. `pnpm verify:skeleton`
(`scripts/verify-software-skeleton.ts`) proves all of it: the files exist, every generated
source parses under the real TypeScript front end, the composition root hardcodes no
section, the spec is derived from the contract, and hand edits are reported the right way
(dropping a reading is an error, dropping a control is a warning, an unknown kind is info).
With `--out <dir>` it writes the project somewhere you can `npm install && npm run dev`.

**Running it is a deliberate, observable act.** The generator still installs nothing — but
the site has a terminal dock (`src/lib/terminal`, `src/components/workspace/TerminalDock.tsx`)
that does, when the user asks for it:

1. the build's checks pass (validation gate closed, run finished, firmware exists);
2. the dock **opens itself**;
3. the user picks a folder — the picker reads the *server's* filesystem one level at a time
   (`GET /api/terminal/browse`), because a browser cannot hand a server a path any other
   way, and every row reports whether it has a `package.json`, a `node_modules` and a `dev`
   script;
4. if that folder has no project in it, the generated website is written into it
   (`POST /api/projects/:id/software/write` — the same bytes as the zip);
5. `npm install` runs, then `npm run dev` (or the pnpm/yarn/bun equivalents the folder's
   lockfile implies, and the `dev` script its manifest declares), each step's output
   streaming into the dock over SSE;
6. the moment the dev server prints its URL the dock links it and `/simulation`'s Website
   half has something to frame.

A failing step stops the sequence instead of running the next command against a half-installed
folder, and stopping a session signals the whole **process group** — `npm run dev` forks Vite,
so killing only npm would leave a server holding the port with nothing attached to it.
`pnpm verify:terminal` (`scripts/verify-terminal.ts`) spawns real children to prove the guard,
the ordering, the failure behaviour and the group kill (it re-binds the port afterwards to
show the forked server really died); with `--site <dir>` it runs a generated website end to
end: install → serve → stop.
...
| `GET /api/projects/:id/simulation` | `{ config, velxio, software, blocked }` — the .vlx to push on the canvas plus the generated website's contract, **surface spec**, file list and static findings |
| `GET /api/projects/:id/simulation/software.zip` | The generated website as a zip (sources + `VALIDATION.md` + the `.vlx`); served even when static checks failed, with the findings in the archive and in `X-Wireup-Validation` |
| `POST /api/projects/:id/software/write` | Body `{ cwd, overwrite? }` → writes the generated website into an empty picked folder or `<picked>/<slug>`, and returns the command sequence to run there; `409 folder_not_empty` over an existing manifest unless `overwrite` |
| `GET /api/terminal/browse?path=` | One directory level of the server's filesystem for the folder picker: `{ path, parent, roots, entries[], current, error }`, each entry flagged with `isProject`, `hasNodeModules`, `devScript`, `installCommand` |
| `POST /api/terminal/sessions` | Body `{ cwd, projectId?, commands?, skipInstallIfPresent? }` → `201` with the session, or `200` when it rejoins one already running in that folder; `400 folder_not_allowed`, `409 session_not_started` |
| `GET /api/terminal/sessions` | `{ enabled, roots, freeform, sessions[] }` |
| `GET /api/terminal/sessions/:id` | `{ session, sessions }`; `session: null` when the server restarted and lost it |
| `GET /api/terminal/sessions/:id/stream?tail=N` | SSE: `snapshot`, `line`, `state`, `closed` |
| `POST /api/terminal/sessions/:id/input` | Body `{ text }` → writes to the running process's stdin; `409 no_stdin` when nothing is waiting |
| `DELETE /api/terminal/sessions/:id` | Stops the session and signals its whole process group |
